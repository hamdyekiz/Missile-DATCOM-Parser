...
# Prepare header
csv_rows = []
header = ["XCG","ALPHA", "MACH", "Q", "CN", "CM", "CA", "CY", "CLN", "CLL",
        ...
        for xcg in xcg_values:
            ...
            for line in original_lines:
                if line.strip().startswith("MACH"):
                    ...
                    prefix, _ = line.split("=", 1)
                    new_line = f"{prefix}= {format(xcg, '.2f')},\n"
                    new_lines.append(new_line)
                else:
            ...
            # Run DATCOM
            print(f"Running datcom97.exe with ALPHA = {alpha}, MACH = {mach}, XCG = {xcg}")
            subprocess.run(["datcom97.exe"], check=True)

            # Extract data
            dyn_q, aero_coeffs = parse_for006(output_filename)
            row = [format(xcg, ".3f"), format(alpha, ".3f"), format(mach, ".3f"), format(dyn_q, ".3f")]
...
# Inside the alpha loop (replace the old block)
for xcg in xcg_values:
    xcg_str = format(xcg, ".3f")
    xcg_dir_path = os.path.join(output_folder, f"xcg{xcg_str}")
    try:
        if os.path.exists(xcg_dir_path):
            os.remove(xcg_dir_path)
    except PermissionError:
        print(f"ERROR: File {xcg_dir_path} is in use. Please close it and re-run.")
        continue
    os.mkdir(xcg_dir_path)
    for alpha in alpha_values:
        alpha_str = format(alpha, ".3f")
        file_path = os.path.join(xcg_dir_path, f"alpha{format(alpha, '.2f')}.csv")

        with open(file_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(header)
            rows_copy = [row for row in csv_rows[1:] if row[1] == alpha_str and row[0] == xcg_str]
